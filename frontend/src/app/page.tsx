...
import { Card, Form, InputNumber, Select, Button, Typography, Row, Col, Divider } from "antd";
import {
  ArrowRightOutlined,
  AimOutlined,
  BarChartOutlined,
...
const features = [
  {
    icon: <DatabaseOutlined style={{ fontSize: 22, color: "var(--zy-secondary)" }} />,
    bg: "rgba(37, 99, 235, 0.08)",
    title: "海量数据支撑",
    desc: "整合全国 3000+ 院校历年录取分数线、位次数据，数据驱动精准匹配",
  },
  {
    icon: <BarChartOutlined style={{ fontSize: 22, color: "var(--zy-accent)" }} />,
    bg: "rgba(5, 150, 105, 0.08)",
    title: "六维画像评估",
    desc: "基础·家庭·城市·兴趣·能力·价值观，全方位评估院校适配度",
  },
  {
    icon: <SafetyCertificateOutlined style={{ fontSize: 22, color: "var(--zy-rush)" }} />,
  ...
  {
    icon: <ThunderboltOutlined style={{ fontSize: 22, color: "var(--zy-primary)" }} />,
    ...
    <AppLayout>
      ...
      <div style={{ textAlign: "center", padding: "16px 0 40px" }}>
        ...
        >
          ...
          <span style={{ fontSize: 13, color: "var(--zy-secondary)", fontWeight: 500 }}>
            AI 驱动的高考志愿智能推荐平台
          </span>
        </div>
        <Title
        ...
        >
          基于 37 万+ 真实录取数据，结合六维画像分析，为你智能生成冲/稳/保梯度志愿方案
        </Paragraph>
        ...
        >
          {stats.map((s) => (
            <div key={s.label} style={{ textAlign: "center" }}>
              ...
              <div style={{ fontSize: 13, color: "var(--zy-text-muted)", marginTop: 4 }}>
                {s.label}
              </div>
            </div>
          ))}
      ...
      <div
        style={{
          ...
          borderRadius: 16,
          padding: "36px 40px",
          boxShadow: "0 4px 24px rgba(30, 58, 95, 0.08), 0 1px 3px rgba(15, 23, 42, 0.06)",
          border: "1px solid var(--zy-border-light)",
        }}
      >
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <Title level={4} style={{ margin: "0 0 6px" }}>快速获取推荐方案</Title>
        ...
        <Form layout="vertical" onFinish={onFinish} size="large">
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="subject_type" label="首选科目" initialValue="物理类" rules={[{ required: true, message: "请选择首选科目" }]}>
      ...
      <Divider style={{ margin: "0 0 40px" }}>
        <Text type="secondary" style={{ fontSize: 13 }}>平台能力</Text>
      </Divider>
        ...
        {features.map((f) => (
          <Col xs={24} sm={12} md={8} key={f.title}>
            ...
            >
              ...
              <Title level={5} style={{ margin: "0 0 8px", fontSize: 15 }}>{f.title}</Title>
              <Text style={{ color: "var(--zy-text-secondary)", fontSize: 13, lineHeight: 1.7 }}>
                {f.desc}
              </Text>
            </Card>
          </Col>
