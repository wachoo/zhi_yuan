...
export default function ProfilePage() {
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (data.basic_info) {
          ...
          initialValues.professional_score = (
            data.basic_info as Record<string, unknown>
          ).professional_score;
        ...
        if (data.personality) {
          ...
          initialValues.dislikes = (
            data.personality as Record<string, unknown>
          ).dislikes;
        }
        if (data.family_info) {
          initialValues.tuition_max = (
            data.family_info as Record<string, unknown>
          ).tuition_max;
          initialValues.prefer_city = (
            data.family_info as Record<string, unknown>
          ).prefer_city;
          initialValues.income_range = (
            data.family_info as Record<string, unknown>
          ).income_range;
          initialValues.parent_industry = (
            data.family_info as Record<string, unknown>
          ).parent_industry;
          initialValues.parent_education = (
            data.family_info as Record<string, unknown>
          ).parent_education;
          initialValues.hukou_type = (
            data.family_info as Record<string, unknown>
          ).hukou_type;
          initialValues.has_siblings = (
            data.family_info as Record<string, unknown>
          ).has_siblings;
          initialValues.has_elderly_care = (
            data.family_info as Record<string, unknown>
          ).has_elderly_care;
          initialValues.home_province = (
            data.family_info as Record<string, unknown>
          ).home_province;
          initialValues.home_city = (
            data.family_info as Record<string, unknown>
          ).home_city;
          initialValues.home_district = (
            data.family_info as Record<string, unknown>
          ).home_district;
        }
        if (data.ability) {
          initialValues.strong_subjects = (
            data.ability as Record<string, unknown>
          ).strong_subjects;
          initialValues.social_ability = (
            data.ability as Record<string, unknown>
          ).social_ability;
          initialValues.english_level = (
            data.ability as Record<string, unknown>
          ).english_level;
        }
        if (data.values_info) {
          initialValues.career_values = (
            data.values_info as Record<string, unknown>
          ).career_values;
          initialValues.distance_preference = (
            data.values_info as Record<string, unknown>
          ).distance_preference;
          initialValues.plan = (
            data.values_info as Record<string, unknown>
  ...
  const doSave = async (values: Record<string, unknown>) => {
    ...
    try {
      ...
      message.success(
        `画像已更新，完整度: ${Math.round(res.data.completeness * 100)}%`,
      );
      setProfile((prev) =>
        prev ? { ...prev, completeness: res.data.completeness } : prev,
      );
    } catch {
      message.error("保存失败");
    } finally {
          ...
          >
            ...
            {examType && (
              ...
              >
                {examType === "普通类" && (
                  <>
                    普通类考生可报考工学、理学、医学、经济学、管理学、法学、文学、历史学、哲学、农学等学科门类的专业，覆盖大部分本科专业。
                  </>
                )}
                {examType === "艺术类" && (
                  <>
                    艺术类考生需参加省统考或校考，可报考视觉传达设计、音乐学、美术学、表演、播音与主持艺术、舞蹈学、动画、广播电视编导等艺术类专业。
                  </>
                )}
                {examType === "体育类" && (
                  <>
                    体育类考生需参加体育专业测试，可报考体育教育、运动训练、武术与民族传统体育等专业。部分院校体育教育也可通过普通类报考。
                  </>
                )}
              </div>
            )}
            {(examType === "艺术类" || examType === "体育类") && (
              <Form.Item
                ...
                extra={
                  examType === "艺术类"
                    ? "省级艺术类专业统考成绩（满分因省份而异，通常300分）"
                    : "省级体育专业统考成绩（满分因省份而异，通常100-400分）"
                }
                style={{ marginTop: 12 }}
              >
                <InputNumber
          ...
          >
            ...
            <Row gutter={24}>
              <Col xs={24} sm={12}>
                <Form.Item
                  name="social_ability"
                  label="社交能力（1=内向，5=外向）"
                >
                  <Slider
                    min={1}
                    max={5}
                    marks={{ 1: "内向", 2: "", 3: "适中", 4: "", 5: "外向" }}
                  />
                </Form.Item>
              </Col>
              <Col xs={24} sm={12}>
